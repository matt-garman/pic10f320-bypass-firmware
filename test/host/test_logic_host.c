...
//
// These tests drive the model through step() (test/model_step.h), which
// delegates to the model's real debounce_integrate() / debounce_step() /
// debounce_init_context(). So this exercises the actual model code -- the same
// code the firmware<->model equivalence test (make test-equiv) pins the real
// PIC10F320 firmware against -- not a second reimplementation.
//
// Build & run: see the Makefile `test-host` target. In short, compile this file
// together with test/model/bypass_pure.c, with -Itest and -Itest/model, then run
// the resulting build_pic/test_logic_host binary.
//
// The thresholds (RELEASE_THRESH / PRESSED_THRESH) come from the model's
// bypass_config.h; the equivalence test enforces that they match the firmware.
...
#include "model_step.h"    // step(), state_t, and the model's state enums
...
//////////////////////////////////////////////////////////////////////////////
// Model wrapper: delegates every step to the vendored reference model
// (test/model/bypass_pure.c) via step(). No algorithm is reimplemented here.
//////////////////////////////////////////////////////////////////////////////

// RELEASE_THRESH and PRESSED_THRESH come from the model's bypass_config.h.
// The model's state enums (PRESS_DEBOUNCE_WAIT, BYPASS, ...) come via
// model_step.h -> bypass_pure.h -> bypass_types.h.
...
// Initialize the model the way the firmware's init() does, delegating to the
// model's debounce_init_context(). pressed_at_power_on mirrors the "footswitch
// held during power-on" case.
static void model_init(model_t *m, int pressed_at_power_on) {
    pin_state_t const pin = pressed_at_power_on ? PIN_STATE_LOW : PIN_STATE_HIGH;
    debounce_context_t const c = debounce_init_context(pin);
    m->program_state    = c.program_state;
    m->effect_state     = c.effect_state;
    m->debounce_counter = c.debounce_counter;
    m->toggle_count     = 0u;
}

// Advance the model by one millisecond via the model's pure step(): the
// saturating integrator, then one state-machine pass -- exactly the per-tick
// work the firmware main() loop body does. pin_low != 0 == switch pressed.
static void model_step_ms(model_t *m, int pin_low) {
    state_t const s = {
        (uint8_t)m->program_state, (uint8_t)m->effect_state, m->debounce_counter
    };
    step_result_t const r = step(s, pin_low);
    m->program_state    = (program_state_t)r.next.program_state;
    m->effect_state     = (effect_state_t)r.next.effect_state;
    m->debounce_counter = r.next.debounce_counter;
    if (r.toggled) { m->toggle_count++; }
}
